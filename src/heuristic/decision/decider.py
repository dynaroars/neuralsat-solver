...
class Decider:
    ...
    def _get(self, unassigned_variables):
        # print(unassigned_variables)
        candidates = set(self.layers_mapping[self.reversed_layers_mapping[unassigned_variables[0]]]).intersection(set(unassigned_variables))
        # print(candidates)
        
        scores = [(n, self.get_score(n)) for n in candidates]
        scores = sorted(scores, key=lambda tup: tup[1], reverse=True)
        node = scores[0][0]
        # print(scores)
        # print(node)
