...
def read_vnnlib_simple(vnnlib_filename, num_inputs, num_outputs, regression=False, is_nhwc=False, input_shape=None):
    ...
    for rv_tuple in merged_rv.values():
        ...
        spec_list = []
        if is_nhwc:
            tmp_box = torch.tensor(box, dtype=torch.float64)
            n_, c_, h_, w_ = input_shape
            tmp_box = tmp_box.view(n_, h_, w_, c_, -1)
            tmp_box = tmp_box.permute(0, 3, 1, 2, 4)
            tmp_box = tmp_box.contiguous().view(-1, tmp_box.shape[-1])
            assert torch.all(tmp_box[:, 0] <= tmp_box[:, 1])
            box = tmp_box.numpy().tolist()
