...
class BatchDeepPoly(nn.Module):
    ...
    def _build_network_transformer(self):
        ...
        layers = [last]
        idx = 0
        for i, layer in enumerate(list(self.net.layers.modules())[1:]):
            if isinstance(layer, nn.Linear):
                ...
                continue
            elif isinstance(layer, onnx2pytorch.operations.base.OperatorWrapper):
                if layer.forward == torch.sub and self.net.dataset == 'acasxu':
                    continue # skip subtract to 0
            else:
                print(type(layer))
    ...
    # @torch.no_grad()
    def __call__(self, lower, upper, adaptive=True, assignment=None, return_hidden_bounds=False, reset_param=False):
        bounds = (lower, upper)
        hidden_bounds = []
        invalid_batch = []
        if reset_param:
        ...
        for layer in self.layers:
            ...
            bounds, _ = layer(bounds, assignment, adaptive)
            # print(bounds)
...
class BatchInputHiddenTransformer(nn.Module):
    ...
    def forward(self, bounds, assignment, adaptive):
...
class BatchReLUTransformer(nn.Module):
    ...
    def forward(self, bounds, assignment, adaptive):
        ...
        if (not torch.is_grad_enabled() or flag_init_step) and adaptive:
